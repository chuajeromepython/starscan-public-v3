...
public interface ClassDao {
  ...
  @Query("SELECT * FROM classes WHERE classroom_id = :classroomId AND teacher_id = :teacherId LIMIT 1")
  ClassEntity getByClassroomIdAndTeacher(int classroomId, int teacherId);
  ...
  @Query("SELECT c.id AS id, c.grade AS grade, c.section AS section, "
          ...
          + "c.created_at DESC")
  List<ClassListRow> queryClassList(int teacherId, String search, String gradeFilter, String schoolYearFilter,
                                    String sortKey);
